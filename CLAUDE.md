...
`scripts/build-legal-pages.mjs` renders them into `public/privacy.html` and
`public/terms.html`, which are **gitignored and regenerated by `predev` and
`prebuild`** — there is no committed copy that can drift from the markdown.
Edit the markdown, never the generated HTML.

- They are plain files rather than routes in the app on purpose: they need no
  session and no database, so they still work when the app does not, and they
  cost the bundle nothing.
- `vercel.json` sets `cleanUrls`, which serves them at `/privacy` and
  `/terms` in production. A small dev middleware in `vite.config.ts` does the
  same locally — without it Vite's SPA fallback answers `/privacy` with
  index.html, so the link renders the app instead of the policy, in dev only.
- Both are linked from the signup form, which is what Art 13 asks for, and
  from Settings.

**The 16+ minimum is enforced in `validateForm`, on signup step 2 only.**
`MINIMUM_AGE` in `AuthPage` feeds the check, the input's `max` and the
wording, so they cannot disagree. Validating it on step 1 makes signup
impossible: the field is not on that step, so the error has nowhere to render
and Next just stops working. `ageInYears` counts the birthday as reached on
the day itself and returns undefined rather than 0 for a missing date, so
"no date" cannot pass a `>= 16` test.
...
1. Privacy policy and terms are **drafted but not in force**, and neither
   should be published until a solicitor has read them. Three fields are
   deliberately left as marked placeholders: the controller's legal name, a
   contact address, and the Supabase project region. The retention schedule
   in section 8 is a proposal, not something verified against the plan. Still
   unsettled beyond the documents: a breach plan and DPAs with Supabase,
   Vercel and Daily. Decided and now written in: 16+, UK and EU GDPR both,
   England and Wales law with the consumer carve-out.
